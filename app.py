...
def main():
    ...
    if classifier == 'Support Vector Machine (SVM)':
        ...
        C = st.sidebar.number_input(
            "C (Strength of Regularization)", 0.01, 100.0, step=0.01, key="C")
        kernel = st.sidebar.radio(
            "Kernel for SVM Classification", ("rbf", "linear"), key="kernel")
        gamma = st.sidebar.radio(
            "Kernel Coefficient (Gamma)", ("scale", "auto"), key='gamma')
        ...
        if st.sidebar.button("Classify, Get Results!"):
            ...
            st.write("Precision: ", precision_score(
                y_test, y_predicted, labels=class_names).round(2))
            st.write("Recall: ", recall_score(
                y_test, y_predicted, labels=class_names).round(2))
            st.write("f1 score: ", f1_score(
                y_test, y_predicted, labels=class_names).round(2))
            st.write("ROC: Area Under the Curve: ", roc_auc_score(
                y_test, y_predicted, labels=class_names).round(2))
    ...
    if classifier == 'Logistic Regression':
        ...
        C = st.sidebar.number_input(
            "C (Strength of Regularization)", 0.01, 100.0, step=0.01, key="C_log")
    ...
    if classifier == 'Random Forest Classification':
        ...
        n_estimators = st.sidebar.number_input(
            "The number of trees in the forest:", 1, 20,  step=1, key='n_estimators')
        max_depth = st.sidebar.number_input(
            "The maximum Depth of Each Tree: ", 1, 20, step=1, key='max_depth')
